...
void AMom::LaserTrace () {
    ...
    FVector StartPos = GetMesh() -> GetSocketLocation(MuzzleSocket);
    ...
    if (Hit.bBlockingHit)
    {
        ...
        if (Hit.GetActor()->IsA(AMyProjectCharacter::StaticClass())) {
            AMyProjectCharacter* character = Cast<AMyProjectCharacter>(Hit.GetActor());
            if (character) {
